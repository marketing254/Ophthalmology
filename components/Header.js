...
// Mirrors the UI draft's information architecture:
// Resources · Podcast · Marketing · Ask AI · Participate · About · [Contact]
const NAV = [
  { href: '/about', label: 'About' },
  { href: '/podcast', label: 'Podcast' },
  { href: '/webinars', label: 'Events' },
  { href: '/resources', label: 'Resources' },
  { href: '/guest-speaker', label: 'Participate' },
  { href: '#ai-summary', label: 'Ask AI' },
  { href: '/marketing', label: 'Marketing', highlight: true },
];
...
export default function Header() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // A clicked dropdown trigger keeps :focus-within, which pins the menu
      // open; release it as soon as the user scrolls away.
      const el = document.activeElement;
      if (el && el.closest && el.closest('.has-dd')) el.blur();
    };
    onScroll();
    ...
    <>
      ...
      <header className={`site${scrolled ? ' scrolled' : ''}`}>
            ...
            {NAV.map((item) =>
              item.children ? (
                ...
                <Link
                  key={item.href}
                  href={item.href}
                  className={`${isActive(item.href) ? 'active' : ''}${item.highlight ? ' nav-mk' : ''}`.trim() || undefined}
                >
                  {item.label}
                </Link>
              )
        ...
        <nav className={`mobile-nav${open ? ' open' : ''}`} aria-label="Mobile">
          ...
          <Link href="/contact" className="m-cta">Contact →</Link>
        </nav>
      </header>
    </>
  );
